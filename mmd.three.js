...
Material = function( bin ) {
	...
	/* // debug
	var n = 5, str = '', flags = this.drawFlags;
	while ( n-- ) {
		str += [ '0 ' , '1 ' ][ (flags & 1) ];
		flags >>= 1;
	}
...
PMX.prototype.createMesh = function( param, oncreate ) {
	...
	monitor.add( function() {
		...
		mesh = new THREE.SkinnedMesh( geo, new THREE.MeshFaceMaterial( materials ) );
		oncreate( mesh );
	});
	...
	createTexture = function( fname ) {
		...
		if ( ext === 'dds' ) {
			...
			texture = THREE.ImageUtils.loadTexture( param.texturePath + fname, undefined,
				function( texture ) {
					if ( Misc.extractPathExt( fname ).toLowerCase() === 'png' ) {
						monitor.add();
						loadBuffer( param.texturePath + fname, function( xhr ) {
...
Reader.prototype.readCString = function( length ) {
	...
	text = Encoding.codeToString( Encoding.convert( b.subarray( 0, i ), 'UNICODE', 'SJIS') );
...
MMDPhysi.prototype.reset = function() { // 初期位置と回転を剛体に設定してリセット。
	var mesh;
	mesh = this.mesh;
	mesh.updateMatrixWorld(); // いつ呼ばれるか分からないからこれ必要。
	_q2.setFromRotationMatrix( mesh.matrixWorld );
	mesh.geometry.MMDrigids.forEach( function( v ) {
		var body;
...
var dv, dq, q0, _v, _v2, _q, _q2, Target;
...
// identity quaternion
q0 = new THREE.Quaternion(0,0,0,1);
	...
	target.bones.forEach( function( v ) {
		...
		if ( at && at[0] >= 0 ) {
			...
			if ( ( v.pmxBone.flags & 0x100) !== 0 ) {
				...
				if ( weight >= 0) {
					// 順回転。
					THREE.Quaternion.slerp( q0, dq, _q, weight );
				} else {
					// 逆回転。
					THREE.Quaternion.slerp( q0, dq.conjugate(), _q, -weight );
				}
...
var skinnedMesh_updateMatrixWorld;
...
var models, cameraMotion, lightMotion,
	motionPlaying, motionLoop, motionDelta, tickPhysics, updatePhysicsDuringPause,
